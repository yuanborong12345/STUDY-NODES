...
3. **相安无事**：迭代器手里的“旧数组”自始至终都没有被改变过，所以它会高高兴兴地把旧数据遍历完。
...
扩容后的容量，是原来数组的1.5倍，通过位运算 `int newCapacity = oldCapacity + (oldCapacity >> 1);`
...
并发场景下，使用CopyOnWriteArrayList ，或者使用Collections.synchronizedList() 包装
...
在Java 8 后，HashMap的底层数据结构是 数组 + 链表 + 红黑树
