...
public class PatternProperties {
    private String dateformat;  //属性，与配置文件一致，会自动注入属性
}
```
...
![image-20210714174728042](images/image-20210714174728042.png)
...
![image-20251108160646872](images/image-20251108160646872.png)
...
| 类型                   | 作用             | 说明                                                         |
| ---------------------- | ---------------- | ------------------------------------------------------------ |
| **feign.Logger.Level** | 修改日志级别     | 包含四种不同的级别：NONE（默认）、BASIC（记录请求基本信息）、HEADERS（请求基本信息，请求头等）、FULL（记录完整的请求信息） |
  ...
  cloud:
    gateway:
      ...
      - id: user-service 
        ...
        predicates: 
        - Path=/user/**
      default-filters: # 默认过滤项,会对所有路由都生效
      - AddRequestHeader=Truth, Itcast is freaking awesome! # 添加请求头 
```
...
@Order(-1) //顺序注解, 越小优先级越高
@Component //定义为spring组件
...
- 路由过滤器和defaultFilter的order由Spring指定，默认是**按照声明顺序从1递增**。
  ...
  ![image-20251108203338290](C:\Users\yuanborong\AppData\Roaming\Typora\typora-user-images\image-20251108203338290.png)
      ...
      globalcors: # 全局的跨域处理
        ...
        corsConfigurations:
          '[/**]':
            ...
            maxAge: 360000 # 这次跨域检测的有效期，有效期内浏览器可以直接发起请求，不再询问服务器，否则不断询问对服务器有压力
```
